...
open class Injector() {
    ...
    open val factoryContainer = FactoryContainer.getSingletonInstance(this, dependencyContainer)
    fun <T : Any> inject(target: KClass<T>, qualifierTag: String? = null): T {
        return getFromContainer(target, qualifierTag) ?: createFromFactory(target, qualifierTag)
    }

    private fun <T : Any> createFromFactory(target: KClass<T>, qualifierTag: String?): T {
        return factoryContainer.getInstance(target, qualifierTag) as T
    }
