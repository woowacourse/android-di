...
class ConstructorInjectionTest {
    ...
    fun `클래스 타입은 등록 없이 자동 생성된다`() {
        // given
        val service =
            dependencyContainer.instance(ServiceDependsOnConcrete::class)
    ...
    fun `생성자 파라미터에 기본값이 있더라도 의존성이 등록되어있지 않으면 생성되지 않는다`() {
        assertThrows(IllegalStateException::class.java) {
            dependencyContainer.instance(ServiceWithDefaultParameter::class)
        }
    }
    ...
    fun `생성자 파라미터에 기본값이 있더라도 의존성이 등록되어 있으면 등록된 의존성을 사용한다`() {
        // given
        dependencyContainer.register(Repository::class) { DefaultRepository() }
        val service = dependencyContainer.instance(ServiceWithDefaultParameter::class)
