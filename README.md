...
- [x] Qualifier 애노테이션으로 같은 타입의 여러 구현체를 구분한다.
- [x] 장바구니 저장소로 Room 구현체와 In-Memory 구현체 중 하나를 선택할 수 있다.
- [x] 같은 타입의 구현체가 여러 개일 때 Qualifier가 없으면 명확한 오류를 낸다.
- [x] Qualifier 해석과 누락 오류를 DI 모듈 테스트로 검증한다.
- [x] DI 코드를 순수 JVM `:di` 모듈로 분리한다.
- [x] `:di` 모듈이 쇼핑 앱의 도메인 타입과 Android 타입을 참조하지 않도록 한다.
