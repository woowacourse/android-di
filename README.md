...
- [ ] DI 라이브러리를 `:di` 모듈로 분리한다.
- [ ] `DependencyContainer`와 Annotation 등 공통 DI 코드를 `:di`로 이동한다.
- [ ] `:di` 모듈이 `CartRepository`, `Product` 등 앱 도메인을 참조하지 않도록 한다.
- [ ] 앱에서 구현체와 의존성을 등록할 수 있도록 구성한다.
...
- [ ] `:di` 모듈이 앱 도메인에 의존하지 않는지 확인한다.
- [ ] 기존 테스트가 모두 통과하는지 확인한다.

### 설계 결정

- [ ] Qualifier를 Annotation과 문자열 키 중 어떤 방식으로 표현할지 결정한다.
- [ ] `:di` 모듈을 순수 JVM 모듈과 Android 모듈 중 어떤 형태로 만들지 결정한다.
- [ ] 선택한 방식과 그 이유를 README에 기록한다.
