...
- [ ] 필드 주입 대상은 애노테이션으로 명시된 필드만이어야 한다. 모든 필드를 훑어 주입하지 않는다.

## 리뷰 체크리스트
- [ ] 장바구니 목록에 상품명과 담은 시각이 함께 표시된다.
- [ ] 애노테이션이 붙은 필드만 주입되고, 붙지 않은 필드는 주입되지 않는다.
- [ ] CartRepository가 CartProductDao를 주입받는다. 즉 의존성이 재귀적으로 해결된다.
- [ ] CartProduct 도메인 모델과 toDomain() 매퍼가 추가되어 있다.
- [ ] ViewModel이 CartRepository의 suspend 함수를 viewModelScope 안에서 호출한다.
- [ ] 목록 중간 항목을 삭제해도 의도한 상품이 지워진다.
- [ ] 필드 주입과 재귀 주입에 대한 테스트가 있다.
- [ ] 사전 제공 테스트가 모두 통과한다.
