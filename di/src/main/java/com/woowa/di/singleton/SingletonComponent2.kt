...
import com.woowa.di.viewmodel.injectViewModelComponentFields
...
class SingletonComponent2<T : Application> private constructor(private val clazz: KClass<T>) :
    ...
    override fun getDIInstance(type: KClass<*>, qualifier: KClass<out Annotation>?): Any? {
        qualifier?.let {
    ...
    private fun createDIInstance(
        ...
        val parameters = kFunc.parameters.filter { it.kind == KParameter.Kind.VALUE }.map {
            when {
                it.hasAnnotation<ApplicationContext>() -> DIBuilder.applicationContext
                else -> SingletonComponentManager2.getDIInstance(
                    it.type.jvmErasure,
        ...
        val instance = if (parameters.isEmpty()) {
