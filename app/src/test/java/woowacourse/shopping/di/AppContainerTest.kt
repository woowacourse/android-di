...
class AppContainerTest {
    ...
    fun `provide로 등록한 인스턴스를 resolve하면 해당 인스턴스가 반환된다`() {
        ...
        AppContainer.provide(ProvidedA::class, provided)

        // when
        val resolved = AppContainer.resolve(ProvidedA::class)
    ...
    fun `provideModule을 호출하면 object의 모든 프로퍼티가 등록된다`() {
        // when
        AppContainer.provideModule(TestModule::class)

        // then
        val ra = AppContainer.resolve(TestModule.ModuleA::class)
        val rb = AppContainer.resolve(TestModule.ModuleB::class)
    ...
    fun `resolve 호출 시 overrides 인스턴스가 우선적으로 사용된다`() {
        ...
        AppContainer.provide(OverFoo::class, provided)

        // when
        val resolved = AppContainer.resolve<NeedsFoo>(mapOf(OverFoo::class to override))
    ...
    fun `MyInjector 어노테이션이 붙은 프로퍼티에 자동으로 인스턴스가 주입된다`() {
        ...
        AppContainer.provide(Repo::class, repoProvided)

        // when
        val holder = AppContainer.resolve(Holder::class)
    ...
    fun `public 생성자가 없는 클래스는 resolve 시 예외가 발생한다`() {
        ...
        assertThrows(java.lang.IllegalArgumentException::class.java) {
            AppContainer.resolve(NoPrimaryConstructor::class)
        }
    }
    ...
    fun `의존성이 다른 의존성을 포함할 때도 중첩 주입이 정상적으로 수행된다`() {
        // when
        val root = AppContainer.resolve(Root::class)
