...
//
//class ViewModelScopedInjector(
//    private val appContainer: DefaultAppContainer,
//) {
//
//    fun <T : Any> createInstance(modelClass: Class<T>) {
//        val kClass = modelClass.kotlin
//
//        val constructor = kClass.primaryConstructor ?: throw IllegalArgumentException(
//            "ViewModel must have a primary constructor: ${kClass.simpleName}"
//        )
//
//        val injectedArgs = constructor.parameters.filter { kParameter ->
...
//            injectedArgs.filter { it.hasAnnotation<ViewModelScope>() }
//                .map { it.type.classifier as KClass<*> to it.withQualifier() }
//        val viewModelScopeFields =
//            injectedFields.filter { it.hasAnnotation<ViewModelScope>() }
//                .map { it.returnType.classifier as KClass<*> to it.withQualifier() }
//
